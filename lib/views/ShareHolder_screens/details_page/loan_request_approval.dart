...
class _LoanRequestDetailsScreenState extends State<LoanRequestDetailsScreen> {
  ...
  Future<void> _submitDecision(ComakerStatus status) async {
    ...
    try {
      ...
      // Fetch fresh data to update UI without closing the page
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            backgroundColor: status == ComakerStatus.rejected ? accentRed : Colors.green,
  ...
  Widget _buildBody(AsyncSnapshot<Map<String, dynamic>> snapshot, LoanRequestModel? loan, ShareholderModel? borrower) {
    ...
    if (loan == null) {
      return const Center(child: Text("Loan data not available."));
    }
      ...
      return Container(
        ...
        child: SafeArea(
          child: Container(
            ...
            child: Text(
              "You already ${status == ComakerStatus.approved ? 'approved' : 'cancelled'} this request.",
