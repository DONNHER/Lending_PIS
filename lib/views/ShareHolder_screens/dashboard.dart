...
class ShareCapitalScreen extends StatelessWidget {
    ...
    return Column(
      ...
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            const Text('Recent Loan Requests',
            ...
            Container(
              ...
              child: Row(
                children: [
                  _buildFilterButton(
                    label: 'Borrower',
                    isSelected: viewModel.loanRequestFilter == 'borrower',
                    onTap: () => viewModel.setLoanRequestFilter('borrower'),
                  ),
                  _buildFilterButton(
                    label: 'Co-maker',
                    isSelected: viewModel.loanRequestFilter == 'comaker',
                    onTap: () => viewModel.setLoanRequestFilter('comaker'),
                  ),
                ],
              ),
            ),
                ...
                ...viewModel.loanRequests.map((req) {
                  return InkWell(
                    ...
                    child: _buildRequestRow(
                        ...
                        req.status.name.toLowerCase() == 'approved' || req.status.name.toLowerCase() == 'released'
                    ),
                  );
                }),
            ],
  ...
  Widget _buildRequestRow(String date, String borrower, String amount, String status, bool isPositive) {
    Color statusColor = Colors.orange;
    if (status.toLowerCase() == 'approved' || status.toLowerCase() == 'released') statusColor = Colors.green;
    if (status.toLowerCase() == 'rejected' || status.toLowerCase() == 'cancelled') statusColor = Colors.red;
    ...
    return Container(
      ...
      child: Row(
        children: [
          Expanded(flex: 2, child: Text(date, style: const TextStyle(fontSize: 11, color: AppTheme.textDark))),
          Expanded(flex: 3, child: Text(borrower, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: AppTheme.textDark))),
          Expanded(
              flex: 2,
              child: Text(amount,
