...
class _LoanPaymentBodyState extends State<_LoanPaymentBody> {
  ...
  Widget _buildActionButtons(BuildContext context, LoanPaymentViewModel viewModel, NumberFormat currencyFormat) {
    return Row(
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            onPressed: viewModel.isSubmitting || viewModel.loan == null
              ...
              // 🛡️ Logic Fix: Prevent payment exceeding remaining balance
              final balance = viewModel.loan?.remainingBalance ?? 0;
              if (amt > (balance + 0.01)) {
                debugPrint('DEBUG [LoanPaymentPage]: Validation failed. Amount $amt exceeds balance $balance.');
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('Payment amount cannot exceed the remaining balance of ${currencyFormat.format(balance)}')),
                );
                return;
              }
